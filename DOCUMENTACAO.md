...
| `vivi-diet-selection.js` | Seleção individual, quantidades e 14 atalhos das refeições prescritas para a Vivi |
...
| `tests/vini-fat-goal.test.mjs` | Testes das médias de saldo, Mifflin–St Jeor, composição-alvo, déficit e projeção do Vini |
| `tests/energy-balance.test.mjs` | Testes dos perfis conservadores de saldo energético, peso por data e compatibilidade histórica |
...
**Como funciona por baixo.** A implementação é compartilhada por `kg-vivi-page.js`; `kg-vini-page.js` é a entrada da página do Vini. O seletor `#kg-section-seg` oferece `Peso`, `Dieta`, `Estatísticas` e `Graphs` para os dois perfis. No Vini, inclui ainda `Academia` e `Corrida`. A seção ativa usa `habitos-kg-section` para Vivi e `habitos-kg-section-vinicius` para Vini.

- **Peso** — `renderWeight()` monta hero, formulário, gráfico, IMC e últimos registros. O registro continua sem horário e separado por usuário. No Vini, peso é a exceção do ciclo iniciado em setembro: hero, gráfico e lista continuam mostrando todo o histórico e a variação desde a primeira pesagem. Os demais números, estatísticas e gráficos são reiniciados pelo filtro do novo ciclo.
...
- **Academia e Corrida do Vini** — `kg-activity-ui.js` reutiliza o mesmo documento diário da página Hábitos. Academia registra os oito grupos musculares já existentes e água de 0,5 L a 5 L. Corrida aceita distância livre, tempo, observações e vínculo opcional com um plano acelerado de 6 etapas rumo aos 5 km em pace-alvo de 5:50/km. O plano foi recalibrado a partir da primeira corrida-base enviada (aprox. 4,74 km totais, 24:12 correndo, blocos próximos de 6:00/km, FC média 137 e máxima 153), começando em blocos de 5–7 min. Cada sessão mostra uma faixa de pace referente somente aos blocos correndo; a caminhada é livre para recuperação. A etapa 5 testa os primeiros 5 km contínuos em 6:00–6:10/km e a etapa 6 testa o alvo de 5:50/km. Cada etapa tem sessões leve, intervalada e longa; o check guarda a data, distância, tempo e pace global efetivamente registrados, sem prender o treino a um dia da semana. Corridas livres entram no histórico e nos gráficos sem avançar a planilha. O botão final salva somente depois das marcações, preservando os demais campos do dia.
- **Dieta da Vivi** — o plano `vivi-nutri-2026-02-v9` preserva as 19 composições oficiais do PDF e usa quatorze atalhos: as seis refeições da dieta base modelada em 03/08/2026, uma dose avulsa de whey com água, 30 g de Palatinose, três dosadores de hipercalórico Growth, três lanches da tarde independentes (1 banana, 1 maçã ou 30 g de castanha-de-caju), café com 100 ml de leite, 18 g de whey e 30 g de Palatinose, e um café de 2 ovos cozidos. O whey deste novo café usa a referência informada de 16 g de proteína em 18 g de produto, separada do whey Probiótica dos demais atalhos. No fim das predefinições há ainda um **Montador rápido** com quatro grupos: carboidratos, proteínas, frutas e suplementos. Cada linha combina um alimento com uma quantidade livre em gramas; o botão `+` acrescenta outra opção do mesmo grupo e `−` remove as linhas extras. Essas escolhas usam os mesmos campos `foods`/`amounts`, entram imediatamente em kcal/macros, gráficos e PDF e são persistidas com o dia. Os atalhos antigos foram removidos apenas da grade de marcação rápida; seus alimentos e as alternativas oficiais continuam disponíveis no menu personalizado e na consulta estática. A hidratação usa a referência prescrita de 35 ml/kg, apresentada no PDF como aproximadamente 1,6 L. O documento não fornece metas clínicas de kcal/macros; por isso as referências de 2.000 kcal, 90 g P, 250 g C e 65 g G continuam explicitamente provisórias.
...
**Estatísticas dos planos.** O dia mostra kcal/macros somados a partir dos alimentos e quantidades marcados, quantidade de alimentos, cobertura dos 4 momentos principais e hidratação. `vini-exercise.js` é compartilhado e usa `(MET − 1) × peso × horas`, com a pesagem mais recente do usuário. O módulo `energy-balance.js` calcula dinamicamente `saldo = ingestão − gasto total estimado`, usando Mifflin–St Jeor, o movimento diário fora dos treinos e o peso mais recente disponível na data. Valor negativo indica déficit e positivo, superávit. Para reduzir falsa precisão, o Vini usa 90% do gasto de rotina e 70% do exercício estimado; a Vivi, 28 anos e 1,63 m, usa 110% da rotina e 100% do exercício, exigindo uma margem maior antes de afirmar superávit. Esses valores não são gravados sobre os snapshots: são recalculados na leitura, preservando todos os registros existentes e o `netKcal` histórico.

No resumo diário, Stats, Graphs e PDF, a métrica vermelha agora é o saldo energético, com linha de equilíbrio em 0 kcal. Proteína, carboidrato e gordura continuam mostrando o consumo ingerido. Os SVGs aceitam valores positivos e negativos, ocupam a largura disponível, reduzem automaticamente os rótulos de data e não criam rolagem horizontal. No Graphs do Vini há também uma linha de água em litros e um gráfico combinado: barras empilhadas por grupo muscular e linha de quilômetros corridos. Esses dois gráficos leem `days`, enquanto os quatro nutricionais leem `diet_logs`; todos respeitam o ciclo selecionado. Sem pesagem, o resumo solicita o peso; a compatibilidade de registros antigos continua usando `netKcal` e, por último, as calorias ingeridas como fallback visual. O PDF é uma página paisagem: no topo reúne evolução de peso e médias de saldo/macros; no centro, organiza os quatro gráficos nutricionais em matriz 2×2; no rodapé, replica o calendário de musculação do mês.

**Contagem regressiva de gordura do Vini.** `vini-fat-goal.js` fixa o ponto informado em 28% no dia 2026-07-15; o alvo começa em 18%, mas pode ser editado entre 5% e 27,9%. A pesagem mais próxima dessa data estima a massa magra inicial e o peso-alvo supondo sua preservação. Logo abaixo do hero, uma barra de progresso ampliada exibe dentro do próprio trilho o percentual do equivalente energético já percorrido. O quadro “Hoje → Objetivo” divide o modelo em gordura, músculo esquelético estimado e outros componentes da massa magra. O músculo usa o modelo antropométrico 2 de Lee et al. (peso, altura, idade e sexo, ajuste racial neutro); “outros” é o resíduo de massa magra, incluindo água, ossos e órgãos. Músculo e outros permanecem constantes no objetivo. O gasto de repouso usa Mifflin–St Jeor para homem, 36 anos e 186 cm; um seletor de movimento fora dos treinos aplica 1,20×, 1,30× ou 1,40×. O déficit de cada registro vem do mesmo saldo conservador do tracker: 90% da rotina mais 70% do treino, menos a ingestão. O mesmo cálculo alimenta o calendário mensal no final do Stats. A energia restante e a gordura estimada já perdida usam 7.700 kcal/kg como aproximação e só consideram dias alimentares registrados; o valor perdido é limitado entre zero e a estimativa total da meta. O card explicita que bioimpedância, metabolismo, massa magra e a própria conversão energética oscilam; é uma tendência recalibrável, não uma previsão clínica.

No topo do Stats, “Médias dos registros” separa a média diária das médias semanal e mensal estimadas (`× 7` e `× 30,44`), sempre usando somente os dias alimentares registrados; kcal representam saldo energético e macros permanecem ingeridos. O simulador mensal de “estrago” usa o déficit diário médio atual e deixa informar quantidade de eventos e kcal **extras** por evento. Ele mostra projeção mensal antes, custo dos eventos, saldo depois, dias equivalentes de déficit e percentual consumido. É uma análise “e se”, não altera registros nem representa uma previsão fisiológica exata. Nível de atividade, percentual-alvo e valores do simulador ficam somente em `localStorage`, sem criar campos no Firebase.

Referências de método: [equação original de Mifflin–St Jeor](https://pubmed.ncbi.nlm.nih.gov/2305711/), [modelo antropométrico de músculo esquelético de Lee et al.](https://pubmed.ncbi.nlm.nih.gov/10966902/) e [Body Weight Planner do NIDDK/NIH](https://www.niddk.nih.gov/health-information/weight-management/body-weight-planner), usado como referência para deixar atividade e incerteza explícitas.
...
`USERS = ["vinicius","victoria"]`, `APP_START_DATE = "2026-05-18"`, `CATEGORY_START_DATES = { academia:"2026-05-18", jiujitsu:"2026-06-08", pilates:"2026-06-09" }`. `TRACKING_CYCLES` mantém a Vivi no ciclo `nutri-2026-07`, iniciado em `2026-07-15`, e inicia o Vini no ciclo `tracking-2026-09` em `2026-09-15`. Ambos seguem sem data final. Projeto Firebase: `gymproject-12fff`.
...
- Estado de UI/sessão: `habitos-auth-uid` (cache do UID logado), `habitos-presence-active-user` (usuário ativo no Status), `habitos-kg-section` (página Peso/Dieta/Stats/Graphs da Vivi), `habitos-kg-section-vinicius` (Peso/Dieta/Academia/Corrida/Stats/Graphs do Vini), `habitos-vini-fat-goal-activity-vinicius` (nível de movimento usado na projeção de gordura), `habitos-vini-fat-goal-target-vinicius` (percentual de gordura desejado), `habitos-vini-fat-goal-damage-days-vinicius` (quantidade de eventos do simulador) e `habitos-vini-fat-goal-damage-kcal-vinicius` (kcal extras por evento). Há ainda a chave legada `habitos-vini-vic`.
