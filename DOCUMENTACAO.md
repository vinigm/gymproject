...
| `vivi-diet-selection.js` | Seleção individual, quantidades e 12 atalhos das refeições prescritas para a Vivi |
...
- **Dieta da Vivi** — o plano `vivi-nutri-2026-02-v8` preserva as 19 composições oficiais do PDF e usa doze atalhos: as seis refeições da dieta base modelada em 03/08/2026, uma dose avulsa de whey com água, 30 g de Palatinose, três dosadores de hipercalórico Growth e três lanches da tarde independentes (1 banana, 1 maçã ou 30 g de castanha-de-caju). No fim das predefinições há ainda um **Montador rápido** com quatro grupos: carboidratos, proteínas, frutas e suplementos. Cada linha combina um alimento com uma quantidade livre em gramas; o botão `+` acrescenta outra opção do mesmo grupo e `−` remove as linhas extras. Essas escolhas usam os mesmos campos `foods`/`amounts`, entram imediatamente em kcal/macros, gráficos e PDF e são persistidas com o dia. Os atalhos antigos foram removidos apenas da grade de marcação rápida; seus alimentos e as alternativas oficiais continuam disponíveis no menu personalizado e na consulta estática. A hidratação usa a referência prescrita de 35 ml/kg, apresentada no PDF como aproximadamente 1,6 L. O documento não fornece metas clínicas de kcal/macros; por isso as referências de 2.000 kcal, 90 g P, 250 g C e 65 g G continuam explicitamente provisórias.
