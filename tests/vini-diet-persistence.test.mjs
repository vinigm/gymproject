...
assert.ok(ui.indexOf("${mealPresetsHTML(day)}") < ui.indexOf("${dailySummaryHTML(summary, energyBalance)}"));
assert.ok(ui.indexOf("${mealPresetsHTML(day)}") < ui.indexOf("${exerciseTrackerHTML(day, summary)}"));
assert.ok(ui.indexOf("${exerciseTrackerHTML(day, summary)}") < ui.indexOf("${dailySummaryHTML(summary, energyBalance)}"));
...
assert.match(ui, /saldo energético · \$\{state\}/);
assert.match(ui, /gasto conservador/);
...
assert.ok(ui.indexOf("${dailySummaryHTML(summary, energyBalance)}") < ui.indexOf("${customFoodsHTML(day, summary)}"));
...
assert.ok(ui.indexOf("${beveragesHTML(day, summary)}") < ui.indexOf("${additionalMealHTML(day)}"));
assert.ok(ui.indexOf("${additionalMealHTML(day)}") < ui.indexOf("${hydrationHTML(day, summary)}"));
assert.match(ui, /id="vini-additional-meal-input"/);
assert.match(ui, /key: "kcal", label: "Calorias"/);
assert.match(ui, /key: "p", label: "Proteína"/);
assert.match(ui, /key: "c", label: "Carboidrato"/);
assert.match(ui, /key: "f", label: "Gordura"/);
assert.match(ui, /data-additional-nutrition="\$\{field\.key\}"/);
