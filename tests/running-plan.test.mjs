...
assert.equal(RUNNING_PLAN.length, 6);
assert.deepEqual(RUN_SESSION_TYPES.map((session) => session.id), ["easy", "interval", "long"]);
assert.match(RUNNING_PLAN[0].sessions.easy.description, /5 min/);
assert.equal(RUNNING_PLAN[0].sessions.easy.pace, "6:15–6:30/km");
...
assert.equal(validRunPlanWeek(7), null);
