...
assert.equal(profile.VINI_PLAN_VERSION, "vivi-nutri-2026-02-v8");
assert.equal(profile.VINI_QUICK_BUILDER_GROUP_IDS.length, 4);
assert.equal(profile.VINI_HYDRATION.baseMl, 1600);
assert.equal(selection.VINI_MEAL_PRESETS[0].id, "base_desjejum");
assert.equal(selection.VINI_MEAL_PRESETS.length, 12);
